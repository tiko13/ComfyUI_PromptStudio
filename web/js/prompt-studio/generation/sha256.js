// Browser-native crypto is unavailable on plain HTTP LAN hosts. Keep identical
// SHA-256 cache keys there; this fallback hashes public workflow metadata only.
...
export function sha256Fallback(bytes) {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index++) {
      ...
      const t1 = (j + s1 + choice + K[index] + w[index]) >>> 0;
      const s0 = rotate(a,2) ^ rotate(a,13) ^ rotate(a,22);
      const majority = (a & b) ^ (a & c) ^ (b & c);
      const t2 = (s0 + majority) >>> 0;
      j=g;g=f;f=e;e=(d+t1)>>>0;d=c;c=b;b=a;a=(t1+t2)>>>0;
    }
    const chunk=[a,b,c,d,e,f,g,j];
    for(let index=0;index<8;index++)h[index]=(h[index]+chunk[index])>>>0;
  }
