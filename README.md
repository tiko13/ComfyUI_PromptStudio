...
With Ollama selected, Prompt Studio uses Ollama's native, non-streaming `/api/chat` endpoint. Sampling controls are translated to Ollama options, and the Thinking control uses Ollama's separate `think` response channel. **Minimal** and **Low** both request Ollama's `low` thinking level. Prompt Studio sizes Ollama's `num_ctx` from the complete system, history, text, image, and response allowance so a small server default cannot silently discard the beginning of the request. If a constrained model spends the initial routing budget on reasoning, Prompt Studio retries that decision once with Thinking disabled and displays a warning instead of failing the Studio turn. A non-structured conversational response that reaches its limit but contains usable text may be retained with an incomplete-result warning; prompt creation, precision revision, routing, proposals, and other machine-applied responses must finish completely. In shared-GPU mode Ollama stays loaded briefly across related LLM stages and is explicitly unloaded at the LLM-to-ComfyUI transition. The ComfyUI canvas nodes remain named KoboldCpp Prompt Slot/Amplify for workflow compatibility; in interactive Prompt Studio, they act as prompt handoff nodes and the provider selected in settings performs the rewrite.
...
**Additional instructions** supplies persistent general steering or explanatory context to the LLM without treating that text as a style or framing modifier. It participates in initial renders, control rebuilds, revisions, and expansion retries, but the latest explicit creation or revision request always remains authoritative. Persistent guidance may refine unspecified details and active controls; it cannot reverse the latest request, expand a precision edit's scope, or import decorative rendered details into the model-neutral main prompt. **Unmodified part** is separate: it bypasses the LLM and passes phrases such as LoRA trigger words unchanged through the workflow's `secondary_instructions` output.
